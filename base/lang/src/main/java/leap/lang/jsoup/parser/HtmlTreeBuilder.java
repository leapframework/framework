...
class HtmlTreeBuilder extends TreeBuilder {
    ...
    Element insert(Token.StartTag startTag) {
        ...
        Element el = new Element(Tag.valueOf(startTag.name(),startTag.isParserCreated()), baseUri, startTag.attributes);
        insert(el);
        return el;
    }

    Element insert(String startTagName) {
    	return insert(startTagName,false);
    }
    
    Element insert(String startTagName,boolean parserCreated) {
        Element el = new Element(Tag.valueOf(startTagName,parserCreated), baseUri);
        insert(el);
        return el;
    }
    ...
    Element insertEmpty(Token.StartTag startTag) {
        Tag tag = Tag.valueOf(startTag.name(),startTag.isParserCreated());
    ...
    void insert(Token.Character characterToken) {
        ...
        Element e = tryGetCurrentElement();
        if(null == e){
        	insertNode(new TextNode(characterToken.getData(), baseUri));
        	return;
        }
        
        // characters in script and style go in as datanodes, not text nodes
        if (StringUtil.in(e.tagName(), TagsScriptStyle))
            node = new DataNode(characterToken.getData(), baseUri);
        else
            node = new TextNode(characterToken.getData(), baseUri);
        
        e.appendChild(node); // doesn't use insertNode, because we don't foster these; and will always have a stack.
