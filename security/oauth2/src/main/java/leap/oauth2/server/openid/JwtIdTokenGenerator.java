...
public class JwtIdTokenGenerator implements IdTokenGenerator {
    ...
    @Override
    public String generateIdToken(OAuth2Authentication authc) {
        return generateIdToken(authc, config.getDefaultIdTokenExpires());
    }

    @Override
    public String generateIdToken(OAuth2Authentication authc, int expires) {
    ...
    protected JwtSigner getJwtSigner(OAuth2Authentication authc, int expires) {
        OAuth2Client client  = authc.getClientDetails();
        
        return new MacSigner(client.getSecret(), expires);
    }
    
    protected Map<String, Object> getJwtClaims(OAuth2Authentication authc, int expires) {
