...
import leap.lang.text.StringCharacterIterator;
 ...
 */
@Internal
public class JerseyUriTemplateParser {
     ...
     */
    public JerseyUriTemplateParser(final String template) throws IllegalArgumentException {
    ...
    private void parse(final StringCharacterIterator ci) {
        try {
            while (ci.next()) {
                char c = ci.current();
                if (c == '{') {
        ...
        if (literalCharactersBuffer.length() > 0) {
            ...
            for (int i = 0; i < s.length(); i++) {
                ...
                if (RESERVED_REGEX_CHARACTERS.contains(c)) {
                    ...
                    if (Hex.isHexCharacter(c1) && Hex.isHexCharacter(c2)) {
    ...
    private void parseName(final StringCharacterIterator ci) {
        char c = ci.nextNonWhitespaceChar();
    ...
    private String parseRegex(final StringCharacterIterator ci) {
