...
public abstract class GenericDbDialect extends GenericDbDialectBase implements DbAware  {
	...
	@Override
    public void setParameter(PreparedStatement ps, int index, Object value, int type) throws SQLException {
		if(null == value || Null.is(value)){
			setNullParameter(ps, index, type);
		}else{
	...
	protected void setNullParameterTypeUnknow(PreparedStatement ps,int index) throws SQLException {
		if(metadata.driverSupportsGetParameterType()){
			ps.setNull(index,ps.getParameterMetaData().getParameterType(index));
		}else{
			nativeSetNullParameterTypeUnknow(ps,index);
...
//			}
	...
	protected void nativeSetNullParameterTypeUnknow(PreparedStatement ps,int index) throws SQLException {
