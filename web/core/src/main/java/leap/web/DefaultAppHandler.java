...
public class DefaultAppHandler extends AppHandlerBase implements AppHandler {
	...
	protected boolean handleByHandlers(Request request,Response response) throws Throwable {
		for(int i=0;i<handlerMappings.length;i++){
			...
			if(mapping.matches(request)){
				if(_debug){
					log.debug("Handling request '{}' by handler '{}'...",request.getPath(),mapping.getRequestHandler().getClass().getName());	
				}
	...
	protected boolean handleError(Request request,Response response, int status, String message, Throwable exception) throws Throwable {
	...
	public boolean handleError(Request request,Response response, Throwable exception) throws Throwable {
		...
		if(request.isAjax()) {
			webConfig.getAjaxHandler().handleError(request, response, status, null == exception ? null : exception.getMessage(), exception);
			return true;
		}
	...
	protected void executeAndRenderAction(Request request,Response response, ActionContext ac, Result result) throws Throwable {
	...
	protected void processResult(ActionContext ac,Validation validation, Object returnValue,Result result) throws Throwable {
