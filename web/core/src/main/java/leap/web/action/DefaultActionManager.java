...
public class DefaultActionManager implements ActionManager {
    ...
    protected Object[] resolveArgumentValues(ActionContext context, Validation validation, RequestFormat format, ExecutionAttributes eas) throws Throwable {
        ...
        if (arguments.length > 0) {
            ...
            values = Arrays2.EMPTY_OBJECT_ARRAY;
        }

        final ActionParams params = new SimpleActionParams(context, arguments, values);
        context.setResolvedParams(params);
        if (arguments.length > 0) {
            final Out<Object> out = new Out<>();
            for (int i = 0; i < arguments.length; i++) {
                final Argument argument = arguments[i];

                Object value = values[i];
                if (null != argument.getProcessor()) {
                    value = argument.getProcessor().postProcessArgument(params, argument, value);
                }
