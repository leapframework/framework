...
enum HtmlTreeBuilderState {
    Initial {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (isWhitespace(t)) {
            	//modified by evan, do not ignroe whitespace
                ...
                DocumentType doctype = new DocumentType(d.getDeclaration(), d.getName(), d.getPublicIdentifier(), d.getSystemIdentifier(), tb.getBaseUri());
    ...
    BeforeHead {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (isWhitespace(t)) {
                ...
                tb.process(new Token.StartTag("head",true));
                return tb.process(t);
            } else if (t.isEndTag()) {
                tb.error(this);
                return false;
    ...
    AfterHead {
        ...
        private boolean anythingElse(Token t, HtmlTreeBuilder tb) {
            tb.process(new Token.StartTag("body",true));
            tb.framesetOk(true);
            return tb.process(t);
        }
    },
            ...
            switch (t.type) {
                ...
                case StartTag:
                    ...
                    if (name.equals("html")) {
                        ...
                        tb.process(new Token.StartTag("form", true));
                        if (startTag.attributes.hasKey("action")) {
                            Element form = tb.getFormElement();
                            form.attr("action", startTag.attributes.get("action"));
                        }
                        tb.process(new Token.StartTag("hr", true));
                        tb.process(new Token.StartTag("label", true));
                        ...
                        tb.process(new Token.StartTag("input", inputAttribs, true));
                        ...
                        tb.insert(startTag);
                        tb.tokeniser.acknowledgeSelfClosingFlag();
                    } else if (tb.isBrowser() && StringUtil.in(name, Constants.InBodyStartDrop)) {
                        tb.error(this);
                        return false;
                    } else {
                        tb.reconstructFormattingElements();
        ...
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (t.isCharacter()) {
                ...
                if (name.equals("caption")) {
                    ...
                    tb.process(new Token.StartTag("colgroup", true));
                    return tb.process(t);
                    ...
                    tb.process(new Token.StartTag("tbody", true));
                    return tb.process(t);
                } else if (name.equals("table")) {
                    tb.error(this);
            ...
            switch (t.type) {
                case StartTag:
                    ...
                    if (name.equals("tr")) {
                        ...
                        tb.error(this);
                        tb.process(new Token.StartTag("tr", true));
                        return tb.process(startTag);
    ...
    AfterBody {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (isWhitespace(t)) {
                //return tb.process(t, InBody);
                tb.insertInFosterParent(t.asCharacter());
            } else if (t.isComment()) {
