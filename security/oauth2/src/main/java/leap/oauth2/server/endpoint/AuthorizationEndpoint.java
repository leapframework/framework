...
public class AuthorizationEndpoint extends AbstractAuthzEndpoint implements SecurityInterceptor {
    ...
    public State postResolveAuthentication(Request request, Response response, SecurityContextHolder context) throws Throwable {
        ...
        Result<OAuth2Client> result = handler.validateRequest(request, response, params);
        if(result.isError()) {
            return State.INTERCEPTED;
        }
        
        OAuth2Client client = result.get();
        ...
        handleAuthenticated(request, response, 
                            new SimpleOAuth2Authentication(params, client, um.getUserDetails(authc.getUserPrincipal()), authc),
                            handler);
    ...
    public State postLoginAuthentication(Request request, Response response, SecurityContextHolder context, Authentication authc) throws Throwable {
        ...
        OAuth2Client client = (OAuth2Client)request.getAttribute(CLIENT_ATTRIBUTE);
        ...
        //Handle authentication.
        handleAuthenticated(request, response, new SimpleOAuth2Authentication(params, client, um.getUserDetails(authc.getUserPrincipal()), authc), handler);
    ...
    protected void handleAuthenticated(Request request, Response response, SimpleOAuth2Authentication authc, ResponseTypeHandler handler) throws Throwable {
    ...
    protected void exposeViewData(Request request, OAuth2Params params, OAuth2Client client) {
    ...
    protected void exposeViewData(Request request, OAuth2Params params, OAuth2Client client, String state) {
        request.setAttribute(CLIENT_ATTRIBUTE, client);
        request.setAttribute(PARAMS_ATTRIBUTE, params);
