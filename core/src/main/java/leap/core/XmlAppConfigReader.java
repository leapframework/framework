...
public class XmlAppConfigReader implements AppConfigReader {
    ...
    protected void readBaseXml(AppConfigReaderContext context, Resource resource) {
    ...
    protected void readFullXml(AppConfigReaderContext context, Resource resource) {
        XmlReader reader = null;
        try{
            reader = XML.createReader(resource);
            reader.setPlaceholderResolver(context.getPlaceholderResolver());
    ...
    private void readBaseProperties(AppConfigReaderContext context, Resource resource, XmlReader reader) {
            ...
            if(reader.isStartElement(IMPORT_ELEMENT)){
                if(matchProfile(context.getProfile(), reader)){
                    ...
                    if(null == importResource || !importResource.exists()){
                        ...
                        context.readImportedResource(importResource, override);
                    }
                }
    ...
    private void processExtensionElement(AppConfigReaderContext context,XmlReader reader){
    ...
    private void readDataSource(AppConfigReaderContext context,Resource resource, XmlReader reader){
        if(!matchProfile(context.getProfile(), reader)){
    ...
    private void readProperties(AppConfigReaderContext context,Resource resource, XmlReader reader){
        if(!matchProfile(context.getProfile(), reader)){
    ...
    private void readPermissions(AppConfigReaderContext context,Resource resource,XmlReader reader){
        if(!matchProfile(context.getProfile(), reader)){
    ...
    private void readPermission(AppConfigReaderContext context,Resource resource,XmlReader reader,boolean granted){
        ...
        try {
            ...
            context.addPermission(new SysPermissionDefinition(reader.getSource(), permType, permObject, granted),context.isDefaultOverrided());
