...
import leap.oauth2.resource.auth.ResAuthenticationManager;
import leap.oauth2.resource.auth.ResAuthentication;
import leap.oauth2.resource.token.ResAccessToken;
import leap.oauth2.resource.token.ResAccessTokenExtractor;
...
public class OAuth2ResourceSecurityInterceptor implements SecurityInterceptor {
    
    //private static final Log log = LogFactory.get(OAuth2ResourceSecurityInterceptor.class);

    protected @Inject OAuth2ResourceConfig       config;
    protected @Inject ResAccessTokenExtractor    tokenExtractor;
    protected @Inject OAuth2ResourceErrorHandler errorHandler;
    protected @Inject ResAuthenticationManager   authcManager;
	
	public OAuth2ResourceSecurityInterceptor() {
        super();
    }
	...
	public State preResolveAuthentication(Request request, Response response, SecurityContextHolder context) throws ServletException, IOException {
		if (config.isEnabled()) {
			...
			//OAuth2ResourcePath url = config.getResourcePath(request.getPath());
	...
	protected State preAuthentication(Request request, Response response, AuthenticationContext context, OAuth2ResourcePath url) throws ServletException, IOException {
