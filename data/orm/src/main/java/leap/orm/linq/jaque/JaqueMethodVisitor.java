...
import leap.orm.linq.jaque.BinaryExpression;
import leap.orm.linq.jaque.ConstantExpression;
import leap.orm.linq.jaque.Expression;
import leap.orm.linq.jaque.ExpressionStack;
import leap.orm.linq.jaque.ExpressionType;
import leap.orm.linq.jaque.TypeConverter;
import leap.orm.linq.jaque.ExpressionStack.BranchExpression;

/**
 * A modified copy of {@link ExpressionMethodVisitor}.
 */
final class JaqueMethodVisitor extends MethodVisitor {
	...
	JaqueMethodVisitor(SerializedLambda sl, MethodNode m) {
		super(Opcodes.ASM5);

		this._argTypes = ASM.getArgumentClassTypes(m);
		this._meVar    = _argTypes.length - 1;
		this._me = Expression.constant(_argTypes[_meVar].getSimpleName(), _argTypes[_meVar]);
	}
	...
	public void visitFieldInsn(int opcode, String owner, String name, String desc) {
		...
		switch (opcode) {
			...
			case Opcodes.GETSTATIC:
				try {
					e = Expression.get(ASM.getClassType(Type.getObjectType(owner)), name);
				} catch (NoSuchFieldException nsfe) {
					throw new RuntimeException(nsfe);
				}
				break;
	...
	public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean itf) {
		...
		for (int i = 0; i < argsTypes.length; i++)
			parameterTypes[i] = ASM.getClassType(argsTypes[i]);
		...
		switch (opcode) {
			...
			case Opcodes.INVOKEINTERFACE:
				try {
					e = Expression.invoke(TypeConverter.convert(_exprStack.pop(), ASM.getClassType(Type.getObjectType(owner))), name, parameterTypes,
					        arguments);
				} catch (NoSuchMethodException nsme) {
					throw new RuntimeException(nsme);
				}
			...
			case Opcodes.INVOKEDYNAMIC:
				try {
					e = Expression.invoke(ASM.getClassType(Type.getObjectType(owner)), name, parameterTypes, arguments);
				} catch (NoSuchMethodException nsme) {
					throw new RuntimeException(nsme);
				}
				break;
	...
	public void visitTypeInsn(int opcode, String type) {
		Class<?> resultType = ASM.getClassType(Type.getObjectType(type));
		Expression e;
		switch (opcode) {
			case Opcodes.NEW:
