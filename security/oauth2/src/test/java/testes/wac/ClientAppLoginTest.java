...
public class ClientAppLoginTest extends OAuth2TestBase {
    ...
    public void testSingleLogin() {
        ...
        assertTrue(redirectBackUrl1.startsWith("https://localhost:8443/clientapp1/oauth2_redirect?oauth2_redirect=1&id_token="));
        get(redirectBackUrl1);
        get("/clientapp1/").assertOk();

        String redirectBackUrl2 = get(redirectUrl2).assertRecirect().getLocation();
        assertTrue(redirectBackUrl2.startsWith("http://localhost:8080/clientapp2/auth_redirect?1=1&oauth2_redirect=1&code="));
