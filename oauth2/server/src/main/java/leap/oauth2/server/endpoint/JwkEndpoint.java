...
public class JwkEndpoint extends AbstractAuthzEndpoint implements Endpoint, Handler {
    ...
    private @Inject JwtSigner signer;

    private @Inject(name = "oauth2PublicKey") @NotNull JwkToken oauth2PublicKey;

    @Override
    public void startEndpoint(App app, Routes routes) throws Throwable {
        if (config.isEnabled()) {
            Stream.of(jwkPath).distinct().forEach(s -> {
                sc.ignore(s);
                routes.create()
    ...
    public void handle(Request request, Response response) throws Throwable {
        JwkWriter.create(request).write(oauth2PublicKey);
    }
